...
class PrsClients
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_prs_search_for_client', [$this, 'search_for_client']);
        add_action('wp_ajax_prs_add_update_user', [$this, 'add_update_user']);
    }

    public function get_clients($filter = 0)
    {
        $clients = $this->get_clients_data();
        if ($clients) {
    ...
    public function get_clients_data()
    {
        ...
        $file = PRS_DATA_FOLDER . 'sync/customers.json';
        $clients = array();
        if (file_exists($file)) {
            $data = json_decode(file_get_contents($file));
            foreach ($data->value as $index => $client) {
                $cclient = array();
                foreach ($client as $key => $value) {
                    switch ($key) {
                        case 'CUSTNAME':
                            $cclient['erp_num'] = $value;
                            break;
                        case 'CUSTDES':
                            $cclient['name'] = $value;
                            break;
                        case 'PHONE':
                            $cclient['Phone1'] = $value;
                            break;
                        case 'EMAIL':
                            $cclient['email'] = $value;
                            break;
